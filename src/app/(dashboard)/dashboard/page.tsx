...
export default function DashboardOverviewPage() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-neutral-950 overflow-y-auto overflow-x-hidden custom-scroll">
      {/* Welcome Banner */}
      <div className="border-b border-neutral-900 bg-neutral-900/10 px-4 py-5 sm:px-8 sm:py-8 shrink-0 select-none relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[220px] h-[100px] sm:w-[500px] sm:h-[200px] bg-cyan-500/5 rounded-full blur-[60px] sm:blur-[100px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-[160px] h-[80px] sm:w-[300px] sm:h-[150px] bg-violet-500/5 rounded-full blur-[50px] sm:blur-[80px] pointer-events-none" />
        <div className="space-y-1.5 relative z-10 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="h-1 w-6 rounded-full bg-gradient-to-r from-cyan-400 to-indigo-500 shrink-0" />
            <span
          ...
          <h1
            className="text-xl sm:text-2xl font-bold text-neutral-100 tracking-tight truncate"
      ...
      <div className="p-4 sm:p-8 max-w-5xl w-full mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 select-none">
          {[
            ...
            <div
              key={label}
              className={`group relative glass glass-card-hover p-3 sm:p-5 flex items-center justify-between gap-2 shadow-lg transition-all duration-300 min-w-0 ${glow}`}
            >
              <div className="space-y-1 min-w-0">
                <span
                  className="block text-[8px] sm:text-[9px] font-bold text-neutral-500 uppercase tracking-widest truncate"
                  style={{ fontFamily: "var(--font-space-grotesk)" }}
                >
                  {label}
                </span>
                <h2
                  className={`text-lg sm:text-2xl font-extrabold truncate ${highlight ? accent : "text-neutral-100"}`}
                  style={{ fontFamily: "var(--font-space-grotesk)" }}
                >
                  {value}
                </h2>
              </div>
              <Icon className={`h-5 w-5 sm:h-6 sm:w-6 shrink-0 ${accent} opacity-50 group-hover:opacity-80 transition-opacity`} />
            </div>
          ))}
        </div>
        <SimpleTodo/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8">
          {/* Recent Notes */}
          <div className="glass glass-card-hover p-4 sm:p-6 space-y-4 min-w-0">
            <div className="flex items-center justify-between gap-2 border-b border-white/[0.12] pb-3 select-none">
              <h3
              ...
              <Link
                href="/notes"
                className="text-[10px] font-bold text-cyan-400 hover:text-cyan-300 hover:underline flex items-center gap-0.5 transition-colors shrink-0"
              >
              ...
              {stats.recentNotes.length === 0 ? (
                ...
                stats.recentNotes.map((note) => (
                  <div
                    ...
                    className="p-3 bg-neutral-950/40 border border-neutral-900 hover:border-neutral-800 rounded-xl flex items-center gap-2 justify-between text-xs transition-all hover:bg-neutral-900/20 group cursor-pointer"
                  >
                    <span className="font-semibold text-neutral-300 group-hover:text-cyan-400 truncate min-w-0 flex-1 transition-colors">
                      {note.title}
                    </span>
                    <span
              ...
              {stats.recentBlogs.length === 0 ? (
                ...
                stats.recentBlogs.map((blog) => (
                  <div
                    ...
                    className="p-3 bg-neutral-955/40 border border-neutral-900 hover:border-neutral-800 rounded-xl space-y-1 transition-all hover:bg-neutral-900/20 cursor-pointer group min-w-0"
                  >
